...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        dest='path_to_csv',
        required=True,
        help='relative path to the workload csvs')
    ...
    for workload in ['readseq', 'readrandom', 'mixgraph', 'readreverse', 'updaterandom']:
        df = pd.read_csv(os.path.join(path, workload+'.csv'))
        pass
    ...
    #
    #     # Column names
        df.rename({f'{workload}_kml': 'kml', f"{workload}_vanilla" : 'vanilla'}, axis=1, inplace=True)
        col_kml = f'kml'  # KML
        col_van = f'vanilla'  # Vanilla
    ...
    #     df['Runtime (minutes)'] = df.reset_index()['index'] / 60
        ...
        plt.savefig(os.path.join(os.curdir, path,
                                 f'{workload}.pdf'), transparent=True)
